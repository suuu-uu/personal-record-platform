...
function PlaceholderDialog({kind,children}:{kind:"achievement"|"footprint";children:React.ReactNode}){const achievement=kind==="achievement";return <Dialog><DialogTrigger asChild>{children}</DialogTrigger><DialogContent><DialogHeader><DialogTitle>{achievement?"添加成就":"选择足迹"}</DialogTitle>{achievement&&<DialogDescription>完整的成就记录表单将在第 3 段接入。你现在可以前往成就页添加记录。</DialogDescription>}</DialogHeader><a className="primary-button" href={achievement?"/achievements":"/footprints"}>{achievement?<Award size={17}/>:<MapPin size={17}/>}前往{achievement?"成就":"足迹"}页</a></DialogContent></Dialog>}
